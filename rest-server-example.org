...
# TODO:  Relativo a  cadastro ou  operações com  uma entidade,  ou com
# elementos que podem não ser entidades.
...
Este método é chamado por uma requisição ~POST~ em ~/users~.
...
#+RESULTS:
: #<STANDARD-METHOD REST-SERVER.DB:CONTROL-STORE ((EQL :USER) T T) {1006C1B623}>
...
#+RESULTS:
: #<STANDARD-METHOD REST-SERVER.DB:CONTROL-STORE ((EQL :SESSION) T T) {1005548213}>
