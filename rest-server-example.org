...
#+RESULTS:
: #<FUNCTION (LAMBDA (PARAMS)) {5335EE0B}>
...
#+RESULTS:
: #<FUNCTION (LAMBDA (PARAMS)) {53361ECB}>
...
Um  método  ~create-from-alist~ toma  uma  certa  chave de  objeto  (por
...
A execução de ~create-from-alist~ efetivamente  deverá *criar o objeto no
banco de dados*.
...
#+RESULTS:
: #<STANDARD-METHOD REST-SERVER.DB:FROM-ALIST ((EQL :USER) T) {1007399093}>
